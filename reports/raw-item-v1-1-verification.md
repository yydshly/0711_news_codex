...
- Web-only 模式从页面创建操作 47，页面立即返回且任务保持 `queued`，明确显示 Worker 尚未领取。
- `newsradar serve` 启动后，操作 47 被 Worker 消费为 `succeeded`，页面可下钻到第 1 次成功 Attempt 和 finished Event。
- 操作 47 对应 FetchRun 显示收到 5 条，RawItem 页面可查看对应 Hacker News 内容。
- 操作 48 从页面成功取消；随后重试创建操作 49，页面显示“由任务 #48 重试创建”，并成功取消测试任务。
- 重复候选页面显示人工确认/排除入口；真实候选没有在验收中被改动，裁决写入由自动化测试覆盖。
- Codex 内置浏览器使用 `Origin: null` 和 `Sec-Fetch-Site: same-origin` 提交本地表单。安全边界现在只在 loopback Host、该精确 Fetch Metadata 组合和一次性 token 同时通过时接受，跨站或缺少上下文仍被拒绝。
...
- `uv run pytest -q`：353 项通过，0 项失败，0 项跳过；仅有已知依赖弃用警告。
...
- 修复内置浏览器的安全同源兼容，同时保持 loopback、Fetch Metadata 和一次性 token 三重约束。
- 抓取批次页面改为优先显示 `items_received`，不再把真实收到 5 条错误显示为旧字段的 0。
- 重试任务详情显示来源任务链接，审计关系可在中文页面直接理解。
